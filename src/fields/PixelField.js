...
export default class PixelField extends Field {
  ...
  orientation() {
    return this.dataset.SharedFunctionalGroupsSequence.PlaneOrientation.ImageOrientationPatient.map(
      Number
    );
  }
  ...
  spacing() {
    let pixelMeasures = this.dataset.SharedFunctionalGroupsSequence
  ...
  analyze() {
    ...
    this.pixelToPatient = mat4.fromValues(
      ...columnStepToPatient,
      0,
      ...rowStepToPatient,
      0,
  ...
  samplingShaderSource() {
    return `
        ...
        vec3 patientToTexture${this.id}(const in vec3 patientPoint)
        {
          ...
          vec3 stpPoint = (patientToPixel${
            this.id
          } * vec4(patientPoint, 1.)).xyz;
        ...
        vec3 textureToPatient${this.id}(const in vec3 stpPoint)
        {
          ...
          vec3 pixelDimensions = vec3(pixelDimensions${this.id});
          vec3 patientPoint = (pixelToPatient${
            this.id
          } * vec4(pixelDimensions * stpPoint, 1.)).xyz;
          return(patientPoint);
        }
                                     ...
                                     out float gradientMagnitude)
        {
          ...
          #define SAMPLE(p) rescale${
            this.id
          }(float( texture(textureUnit, p).r ))
