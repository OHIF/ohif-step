...
export default class TransformField extends PixelField {
  constructor(options = {}) {
    super(options);
    this.analyze();
  }
    ...
    if (needsUpdate) {
      ...
      if (!this.generator) {
        ...
        for (let vectorIndex = 0; vectorIndex < vectorCount; vectorIndex++) {
          ...
          rgbaArray[4 * vectorIndex + 1] =
            grid.VectorGridData[3 * vectorIndex + 1];
          rgbaArray[4 * vectorIndex + 2] =
            grid.VectorGridData[3 * vectorIndex + 2];
